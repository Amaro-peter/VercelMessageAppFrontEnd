...
class CrystalKyBerMlKem1024Provider implements ICryptoProvider {
  private readonly kem = new MlKem1024(); // usa Kyber-1024
  private readonly AES_IV_BYTES = 12; // AES-GCM nonce
  private readonly ALGO_NAME = "CRYSTALS-KYBER (MlKem1024) + AES-GCM";
  ...
  public async generateKeyPair(): Promise<KeyPair> {
    try {
      ...
      console.log(`🔑 Kyber KeyPair gerado - PK: ${pub.length} bytes, SK: ${priv.length} bytes`);
      
      return { publicKey: pub, privateKey: priv };
    } catch (error) {
      console.error("Erro ao gerar par de chaves Kyber:", error);
  ...
  public async encrypt(message: string, recipientPublicKey: Uint8Array): Promise<EncryptedMessage> {
    try {
      ...
      console.log(`🔒 Iniciando criptografia Kyber - Mensagem: ${message.length} chars, PK: ${recipientPublicKey.length} bytes`);
      ...
      console.log(`🔐 Kyber encap concluído - CT: ${ciphertextKEM.length} bytes, Shared: ${sharedSecret.length} bytes`);
      ...
      console.error("Erro na criptografia Kyber:", error);
  ...
  public async decrypt(encryptedMessage: EncryptedMessage, privateKey: Uint8Array): Promise<string> {
    try {
      ...
      console.log(`🔓 Iniciando descriptografia Kyber - Algorithm: ${encryptedMessage.algorithm}`);
      ...
      console.log(`🔐 Kyber decap concluído - Shared secret: ${sharedSecret.length} bytes`);
      ...
      console.log(`✅ Descriptografia concluída com sucesso - Mensagem: ${result.length} chars`);
      return result;
      
    } catch (error) {
      console.error("Erro na descriptografia Kyber:", error);
...
export { CrystalKyBerMlKem1024Provider };
