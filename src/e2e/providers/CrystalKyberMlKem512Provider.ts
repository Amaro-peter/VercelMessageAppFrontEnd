// CrystalKyberMlKem512Provider.ts
// Implementação Kyber (crystals-kyber-js) MlKem512 + AES-GCM (WebCrypto) — Hybrid Encryption (KEM + AEAD).
// Requer: npm install crystals-kyber-js
//
// Notas de segurança:
// - Kyber MlKem512 é um KEM com menor tamanho de chave comparado ao MlKem1024
...
/**
 * CrystalKyberProvider usando MlKem512 de crystals-kyber-js
 */
class CrystalKyBerMlKem512Provider implements ICryptoProvider {
  private readonly kem = new MlKem512(); // usa Kyber-512
  private readonly AES_IV_BYTES = 12; // AES-GCM nonce
  private readonly ALGO_NAME = "CRYSTALS-KYBER (MlKem512) + AES-GCM";
  ...
  public async generateKeyPair(): Promise<KeyPair> {
    try {
      ...
      console.log(`🔑 Kyber MlKem512 KeyPair gerado - PK: ${pub.length} bytes, SK: ${priv.length} bytes`);
      
      return { publicKey: pub, privateKey: priv };
    } catch (error) {
      console.error("Erro ao gerar par de chaves Kyber MlKem512:", error);
  ...
  public async encrypt(message: string, recipientPublicKey: Uint8Array): Promise<EncryptedMessage> {
    try {
      ...
      console.log(`🔒 Iniciando criptografia Kyber MlKem512 - Mensagem: ${message.length} chars, PK: ${recipientPublicKey.length} bytes`);
      ...
      console.log(`🔐 Kyber MlKem512 encap concluído - CT: ${ciphertextKEM.length} bytes, Shared: ${sharedSecret.length} bytes`);
      ...
      console.log(`✅ Criptografia MlKem512 concluída com sucesso`);
      return result;
      
    } catch (error) {
      console.error("Erro na criptografia Kyber MlKem512:", error);
  ...
  public async decrypt(encryptedMessage: EncryptedMessage, privateKey: Uint8Array): Promise<string> {
    try {
      ...
      console.log(`🔓 Iniciando descriptografia Kyber MlKem512 - Algorithm: ${encryptedMessage.algorithm}`);
      ...
      console.log(`🔐 Kyber MlKem512 decap concluído - Shared secret: ${sharedSecret.length} bytes`);
      ...
      console.log(`✅ Descriptografia MlKem512 concluída com sucesso - Mensagem: ${result.length} chars`);
      return result;
      
    } catch (error) {
      console.error("Erro na descriptografia Kyber MlKem512:", error);
...
export { CrystalKyBerMlKem512Provider };